...
export default function QuickClientDialog({ onCreated, triggerLabel = "Tambah klien" }: { onCreated?: (id: number) => void; triggerLabel?: string }) {
  ...
  const create = trpc.clients.create.useMutation({ onSuccess: id => { utils.clients.list.invalidate(); toast.success("Klien ditambahkan."); setOpen(false); setName(""); setEmail(""); setPhone(""); onCreated?.(id); }, onError: error => toast.error(error.message) });
