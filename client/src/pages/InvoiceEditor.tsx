...
export default function InvoiceEditor() {
  ...
  const [discount, setDiscount] = useState(0); const [taxRate, setTaxRate] = useState(11); const [currency, setCurrency] = useState("IDR"); const [notes, setNotes] = useState(""); const [items, setItems] = useState<Line[]>([newLine()]);
  ...
  const save = (status: "draft" | "sent") => {
    ...
    const data = { clientId: Number(clientId), invoiceNumber, invoiceDate: new Date(`${invoiceDate}T12:00:00`), dueDate: new Date(`${dueDate}T12:00:00`), status, currency, discount: Math.round(discount), taxRate: Math.round(taxRate), notes: notes || null, items: validItems.map(item => ({ catalogItemId: item.catalogItemId || null, description: item.description.trim(), quantity: Math.max(1, Math.round(item.quantity)), unitPrice: Math.max(0, Math.round(item.unitPrice)) })) };
