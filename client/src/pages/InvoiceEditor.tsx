...
import { CalendarDays, CircleX, Plus, Save, Send, Trash2 } from "lucide-react";
...
export default function InvoiceEditor() {
  ...
  const [discountType, setDiscountType] = useState<DiscountType>("amount"); const [discountValue, setDiscountValue] = useState(0); const [taxRate, setTaxRate] = useState(11); const [currency, setCurrency] = useState("IDR"); const [notes, setNotes] = useState(""); const [storeNumber, setStoreNumber] = useState(""); const [shippingAddress, setShippingAddress] = useState(""); const [items, setItems] = useState<Line[]>([newLine()]);
  ...
  return <><PageHeader eyebrow={invoiceId ? "Edit invoice" : "Invoice baru"} title={invoiceId ? "Edit invoice" : "Buat invoice baru"} description="Lengkapi informasi klien dan item; nilai tagihan akan dihitung otomatis." /><WorkflowStepper className="mb-6" currentStep={workflowStep} steps={[{ label: "Informasi klien", description: "Pilih klien dan detail tagihan" }, { label: "Item & pengiriman", description: "Isi item, toko, dan alamat" }, { label: "Simpan invoice", description: "Periksa total lalu simpan" }]} />
