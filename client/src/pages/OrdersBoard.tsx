...
export default function OrdersBoard() {
  ...
  return <>
    ...
    {invoices.isLoading ? <div className="grid gap-4 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-72" />)}</div> : <><p className="mb-3 flex items-center gap-2 text-xs font-medium text-slate-500 lg:hidden"><span aria-hidden="true">↔</span> Geser board ke kanan untuk melihat semua tahap pesanan.</p><div className="overflow-x-auto pb-4"><div className="grid min-w-[1720px] grid-cols-6 gap-4">{FULFILLMENT_STATUSES.map(status => {
