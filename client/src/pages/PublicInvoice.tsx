...
export default function PublicInvoice() {
  ...
  return <main className="min-h-screen bg-slate-100 px-3 py-5 sm:px-6 sm:py-8"><div className="no-print mx-auto mb-5 flex max-w-[820px] items-center justify-between gap-3"><div className="flex items-center gap-3"><Button size="sm" variant="outline" className="gap-1.5 bg-white" onClick={goBack}><ArrowLeft className="size-4" />Kembali</Button><div className="flex items-center gap-3"><div className="grid size-9 place-items-center overflow-hidden rounded-lg bg-white text-sm font-bold shadow-sm" style={{ color: data.business.accentColor }}>{data.business.logoUrl ? <img src={data.business.logoUrl} alt="Logo bisnis" className="size-full object-cover" /> : data.business.businessName.slice(0, 1)}</div><span className="hidden text-sm font-semibold text-slate-700 sm:inline">Invoice dari {data.business.businessName}</span></div></div><div className="flex gap-2"><PrintInvoiceDialog compact /><Button size="sm" className="gap-2" disabled={downloading} onClick={async () => { if (!documentRef.current) return; setDownloading(true); try { await downloadInvoicePdf(documentRef.current, data.invoice.invoiceNumber); } finally { setDownloading(false); } }}><Download className="size-4" />{downloading ? "Membuat PDF…" : "Download PDF"}</Button></div></div><div className="no-print mx-auto mb-4 flex max-w-[820px] justify-end"><InvoiceStatusBadge status={data.invoice.status} /></div><div ref={documentRef}><InvoiceDocument data={data} publicView /></div><p className="no-print mx-auto mt-5 max-w-[820px] text-center text-xs text-muted-foreground">Dibuat dengan Faktur</p></main>;
}
