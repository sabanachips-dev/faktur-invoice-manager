...
describe("Cloudflare invoice create router", () => {
  ...
  it("meneruskan pembuatan invoice ke RPC atomik", async () => {
    ...
    const id = await workerRouter.createCaller(context).invoices.create({ clientId: 2, invoiceNumber: "INV-2026-001", invoiceDate: new Date("2026-08-24T00:00:00.000Z"), dueDate: new Date("2026-08-31T00:00:00.000Z"), status: "draft", currency: "IDR", discountType: "amount", discountValue: 0, taxRate: 11, notes: null, storeNumber: null, shippingAddress: null, items: [{ description: "Produk A", quantity: 2, unitPrice: 50000, catalogItemId: null }] });
    expect(id).toBe(41);
