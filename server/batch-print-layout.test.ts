...
describe("batch print A4 layout", () => {
  it("places two compact invoices on each A4 page", () => {
    const pages = groupBatchPrintPages([document("1", 1), document("2", 3), document("3", 2)], "two");
    expect(pages).toEqual([
      { mode: "compact", documents: [document("1", 1), document("2", 3)] },
      { mode: "compact", documents: [document("3", 2)] },
    ]);
  });

  it("keeps invoices with many items on a dedicated full page to prevent truncation", () => {
    const pages = groupBatchPrintPages([document("1", 1), document("2", 4), document("3", 1)], "two");
